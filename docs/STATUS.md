...
- Phase 2 (UI consultant complète, invitations, mots de passe temporaires) — 🟡 boucle coeur opérationnelle : provisionnement d'un cabinet par le Super Admin (`/admin/consultancy-workspaces`, mot de passe temporaire du consultant principal), portefeuille client consultant (`/consultant`, création de client avec compte admin + mot de passe temporaire, révocation), vérifié de bout en bout avec un vrai navigateur (workspace → connexion consultant → changement de mot de passe forcé → création client → connexion admin client). Restent non commencés : prospects/actifs/archivés, missions, échéances, rapports par client, équipe, modèles, marque blanche.
