...
- Phase 1 (commercial/onboarding UI complet, Super Admin) — 🟡 Super Admin webhooks/audit-logs ajoutés (`/admin/webhooks`, `/admin/audit-logs`), s'ajoutent à l'existant (mot de passe oublié, CTA Stripe réels, checklist onboarding, `/admin/organizations`). **Non fait, délibérément** : onboarding piloté par l'API Sirene (jamais appelée en conditions réelles, forme de réponse jamais vérifiée — voir `PLANS.md`), page marketing Landing/Tarifs complète.
...
- Phase 3 (DUERP versions immuables, workflow validation, exports PDF) — 🟡 immuabilité + chaîne de révision réelles (`validatedAt` verrouille `PATCH`, `POST /api/duerp/[id]/revise` crée une nouvelle version `DRAFT` liée via `previousVersionId`, testé), export PDF réel (`GET /api/duerp/[id]/export/pdf`, `pdfkit`, généré à la demande, vérifié en navigateur réel). Au passage : deux boutons de la fiche DUERP (« Exporter PDF », « Valider ») qui n'avaient jamais fonctionné (route inexistante / méthode HTTP non supportée) ont été trouvés et corrigés. **Non fait, délibérément** : méthodes de cotation configurables — voir `PLANS.md` Phase 3 pour la justification (risque de fonctionnalité simulée).
